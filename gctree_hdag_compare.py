...
def main(
    ...
    cforests = [
        gctree.branching_processes.CollapsedForest(
            [gctree.phylip_parse.disambiguate(tree)]
        )
        for tree in trees
    ]
    kwargls = [
    ...
    best_nondag_tree = min(
        cforests,
        key=lambda f: sign_switch(f._forest.optimal_weight_annotate(**ranking_funcs)),
    )

    nondag_best_ll, nondag_best_mutability = (
        best_nondag_tree._forest.optimal_weight_annotate(**ranking_funcs)
    )
