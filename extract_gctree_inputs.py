...
parser = argparse.ArgumentParser(description="Produce gctree inputs from a Partis output directory")
parser.add_argument('input_dir', type=str, help="The partis output directory containing simulations")
parser.add_argument('output_dir', type=str, help="The directory in which to place gctree inputs, in one subdirectory per simulation")
...
for simu_idx, (annotation_h, annotation_k) in enumerate(zip(annotations_h, annotations_k)):
    ...
    k_seq_alignment = {uid: seq for uid, seq in zip(annotation_k['unique_ids'], annotation_k['seqs'])}

    alignment_concat = {}
    for seqid_h, paired_k_seqids, seq_h in zip(annotation_h['unique_ids'], annotation_h['paired-uids'], annotation_h['seqs']):
