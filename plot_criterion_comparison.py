...
for sim_id, simdict in datacounters:
    ...
    def process_rf_counter(rf_nodecount_counter, process_func):
        ...
        for (rf, nodecount), count in rf_nodecount_counter.items():
            newcounter.update({process_func(rf, nodecount, num_true_nodes, num_leaves): count})
        return newcounter
    ...
    for handler, handler_name in rf_handlers:
        for dtype_name, datacounter in counterdict.items():
            ...
            for summary_name, summarizer in counter_summarizers:
                datadict[summary_name + dtype_name + handler_name].append(summarizer(norm_rfcounter))
    ...
    new_dnapars_datafields = tuple([it[1] for it in rf_handlers]) + dnapars_datafields[1:]

    def process_dnapars_tuples(tup):
        rf, infercount = tup[0]
        return tuple(handler(rf, infercount, num_true_nodes, num_leaves) for handler, _ in rf_handlers) + tup[1:]
    ...
    for idx, name in enumerate(new_dnapars_datafields):
        for sum_name, summarizer in dnapars_summarizers:
            datadict["dnapars_" + sum_name + name].append(summarizer([float(t[idx]) for t in new_dnapars_data]))

    for idx, name in enumerate(new_dnapars_datafields):
        for opt in (min, max):
            opt_val = opt(tup[idx] for tup in new_dnapars_data)
            datadict["dnapars_" + opt.__name__ + "_" + name + "_mean_norm_rf"].append(stat.mean(tup[0] for tup in new_dnapars_data if math.isclose(tup[idx], opt_val)))
    
    datadict["dnapars_BestMutabilityMaxLikelihood"].append(min([(-t[2], t[3]) for t in new_dnapars_data])[1])
...
abundance_filter = (pl.col("max_abundance") > 2) & (pl.col("fraction_nonsingleton") > .1)
...
for faceted_df, facet_name in [(df.filter(abundance_filter), "informative_abundance"), (df.filter(abundance_filter.not_()), "uninformative_abundance")]:
...
improvements["max improvement"] = df["minWhole DAGNormalized_rf"] - df["dnapars_minNormalized_rf"]
...
# Create box plot
boxplot = ax.boxplot(improvements.values(), labels=improvements.keys(), patch_artist=True)
...
hdag_comparison_df = hdag_comparison_df.with_columns(
    (-pl.col("trimval_best_ContextLikelihoodLogLoss") + pl.col("dnapars_minContextLikelihoodLogLoss")).alias("Context Likelihood Improvement"),
    (-pl.col("trimval_best_BPLikelihoodLogLoss") + pl.col("dnapars_minBPLikelihoodLogLoss")).alias("Branching Process Likelihood Improvement"),
...
ax1.yaxis.grid(True, linestyle='-', which='major', color='lightgrey', alpha=0.7, zorder=3)
ax2.yaxis.grid(True, linestyle='-', which='major', color='lightgrey', alpha=0.7, zorder=3)
ax1.xaxis.grid(True, linestyle='-', which='major', color='lightgrey', alpha=0.7, zorder=3)
ax2.xaxis.grid(True, linestyle='-', which='major', color='lightgrey', alpha=0.7, zorder=3)
ax1.axhline(y=1, color='lightgrey', linestyle='-', linewidth=1.5, label='Reference (y=1)', zorder=3)
ax2.axhline(y=1, color='lightgrey', linestyle='-', linewidth=1.5, label='Reference (y=1)', zorder=3)
ax1.axvline(x=1, color='lightgrey', linestyle='-', linewidth=1.5, label='Reference (x=1)', zorder=3)
ax2.axvline(x=1, color='lightgrey', linestyle='-', linewidth=1.5, label='Reference (x=1)', zorder=3)
# Plotting the data
ax1.scatter(hdag_comparison_df["Trees Ranked Improvement"].exp(), hdag_comparison_df["Branching Process Likelihood Improvement"].exp(), color=colormaps['Dark2'].colors[0], alpha=.5, s=20, linewidths=0, label='Branching Process Likelihood Improvement', zorder=4)
ax2.scatter(hdag_comparison_df["Trees Ranked Improvement"].exp(), hdag_comparison_df["Context Likelihood Improvement"].exp(), facecolors=colormaps['Dark2'].colors[1], alpha=.5, s=20, linewidths=0, label='Context Likelihood Improvement', zorder=4)
...
for ax in (ax1, ax2):
    ax.yaxis.grid(True, linestyle='-', which='major', color='lightgrey', alpha=0.7, zorder=3)
    ax.xaxis.grid(True, linestyle='-', which='major', color='lightgrey', alpha=0.7, zorder=3)
...
# Categorize data
hdag_comparison_df, prop_above1 = categorize_data(hdag_comparison_df, "Branching Process Likelihood Improvement")
hdag_comparison_df2, prop_above2 = categorize_data(hdag_comparison_df, "Context Likelihood Improvement")
