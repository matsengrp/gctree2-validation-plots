...
hdag_df = hdag_df.rename(
    lambda name: name if name == "ParsimonyForestPath" else "hDAG_" + name
)
...
combined_df = combined_df.with_columns(
    pl.Series(
        ...
        values=[math.log(int(x)) for x in combined_df["hDAG_NumTreesRanked"]],
    )
)
combined_df = combined_df.with_columns(
    pl.Series(
        name="Log_NumTreesRanked",
        values=[math.log(int(x)) for x in combined_df["NumTreesRanked"]],
    )
)

combined_df = combined_df.with_columns(
    (pl.col("Log_hDAG_NumTreesRanked") - pl.col("Log_NumTreesRanked")).alias(
        "LogRatioTreesRanked"
    ),
    (pl.col("hDAG_BestLikelihood") - pl.col("BestLikelihood")).alias(
        "LogRatioBestLikelihood"
    ),
    (pl.col("BestMutability") / pl.col("hDAG_BestMutability")).alias(
        "RatioBestMutability"
    ),  # order swapped so that >1 is improvement with hdag
)

ratio_min, ratio_max = (
    combined_df["RatioBestMutability"].min(),
    combined_df["RatioBestMutability"].max(),
)
...
scatter = ax.scatter(
    ...
    cmap=LinearSegmentedColormap.from_list(
        "not_viridis", ["white", "purple"]
    ),  # You can choose a colormap that suits your data
)
